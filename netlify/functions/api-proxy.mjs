...
export const handler = async (event, context) => {
  ...
  try {
    ...
    const apiKey = bodyApiKey || event.headers['x-api-key'] || process.env.HZM_API_KEY;
    const userEmail = bodyUserEmail || event.headers['x-user-email'] || process.env.HZM_USER_EMAIL;
    const projectPassword = bodyProjectPassword || event.headers['x-project-password'] || process.env.HZM_PROJECT_PASSWORD;
